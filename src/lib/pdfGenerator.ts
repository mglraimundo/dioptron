...
function buildDocDefinition(session: ClinicalSession, provider: ProviderConfig): TDocumentDefinitions {
  ...
  // Header block: logo left (~30pt tall at width 180), title + date right (bottom-aligned)
  ...
  // Unified refraction table
  const refractionTable = makeUnifiedTable(session.od, session.oe);
  ...
  if (session.observacoes) {
    ...
    notesContent.push({ text: capitalize(session.observacoes), fontSize: 12 });
  }
...
export async function generatePdf(session: ClinicalSession, provider: ProviderConfig): Promise<void> {
  ...
  const filename = `receita_${safeName}_${dateForFile}.pdf`;
